...
O botão **Discovery** permite escolher entre duas modalidades. O discovery remoto produtivo usa somente `ListObjectsV2` paginado: registra chave, tamanho, ETag, classe de armazenamento e última modificação sem restaurar, baixar, fazer `HeadObject` ou listar tags. Metadados e tags são lidos somente no momento da cópia de cada objeto já restaurado. A alternativa **Usar arquivo de inventário** importa um CSV UTF-8 (ou `.csv.gz`) diretamente para a origem, em lotes no servidor e sem chamar a AWS. O arquivo deve conter cabeçalho e as colunas `object_key`/`Key` e `size_bytes`/`Size`; também aceita `ETag`, `StorageClass`, `LastModifiedDate`, `VersionId`, `metadata_json` e `tags_json`. A importação só é permitida antes da criação de waves e não é mesclada com um inventário já existente, preservando a rastreabilidade. As ações da console apenas registram ou enfileiram trabalho durável; não causam chamadas AWS pelo navegador.

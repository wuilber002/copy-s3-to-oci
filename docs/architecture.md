...
- Cada wave possui uma estimativa de custo sob demanda. Ela calcula jobs/tarefas Batch, manifests, requests de discovery/polling/leitura, retrieval por classe e tier, retenção temporária, saída AWS, operações multipart OCI e armazenamento mensal. As tarifas são cadastradas por conexão AWS, com referência e data fornecidas pelo operador; uma tarifa ausente torna o componente e o total correspondente explicitamente não estimados, em vez de assumir preço zero.

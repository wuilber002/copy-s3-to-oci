...
- O agendador dinâmico é obrigatório para esse modo e mantém somente um horizonte durável configurável de waves materializadas (três por padrão). Cada ciclo de governança cria a próxima wave apenas quando abre espaço no horizonte, já usando as medições observadas nas anteriores; portanto tamanho, quantidade e horários das futuras waves são recalculados progressivamente. Jobs aceitos, restores e transferências nunca são reescritos. A observabilidade destaca waves cuja cópia prevista pode ultrapassar a expiração da cópia temporária restaurada.

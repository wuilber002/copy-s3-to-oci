...
- O agendador dinâmico é obrigatório para esse modo e mantém somente um horizonte durável configurável de waves materializadas (duas por padrão). Cada ciclo de governança cria a próxima wave apenas quando abre espaço no horizonte, já usando as medições observadas nas anteriores; portanto tamanho, quantidade e horários das futuras waves são recalculados progressivamente. Jobs aceitos, restores e transferências nunca são reescritos. A observabilidade destaca waves cuja cópia prevista pode ultrapassar a expiração da cópia temporária restaurada.
...
- A chave global **Pipeline dinâmico de restores** habilita a criação dinâmica. Uma vez habilitado, o agendamento antecipado é obrigatório: a primeira janela usa somente o limite de serviço do tier (48 h para BULK ou 12 h para STANDARD), e a reserva operacional antecipa a submissão das waves seguintes em relação ao início previsto de sua cópia. O **Horizonte de restores** limita quantas waves dinâmicas podem estar materializadas e com restore liberado em paralelo (padrão: duas). Isso evita acumular cópias temporárias antes da necessidade e deixa o histórico refinar apenas waves ainda não criadas.

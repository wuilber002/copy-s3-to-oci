...
| `<SUBNET_OCID>` | Subnet existente com saída HTTPS para AWS, OCI e repositório de instalação. | `ocid1.subnet.oc1.sa-saopaulo-1.example` |
...
## 3. Preparar rede e acesso administrativo

A subnet é responsabilidade do cliente. Ela deve permitir saída HTTPS (TCP
443) para Amazon S3/STS, endpoints de preço AWS quando essa funcionalidade for
usada, OCI Vault, OCI Object Storage e o repositório de instalação. Essa saída
pode ocorrer por internet ou conectividade privada aprovada pelo cliente.
...
1. Abra **Networking → Virtual cloud networks → <VCN> → Security Lists** ou
   **Network Security Groups**.
2. Confirme a saída TCP 443 necessária.
3. Inclua a regra SSH aprovada pelo cliente.
4. Não crie regra de entrada para TCP 8080.
